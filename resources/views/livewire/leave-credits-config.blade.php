    ...
    <div class="flex-1 h-[1000px] flex flex-col gap-10">
        <div class="flex-1 h-[900px] bg-white rounded-xl p-6 shadow">
            ...
            <table class="min-w-full table-auto text-sm mt-4">
                ...
                <tbody>
                    @for ($i = 0; $i < 30; $i++) <tr class="text-xs hover:bg-gray-100 odd:bg-white even:bg-gray-50">
        ...
        <div class="flex-1 h-[900px] bg-white rounded-xl p-6 shadow">
            ...
            <div class="w-full flex gap-6">
                ...
                <table class="flex-1 table-auto text-sm mt-4">
                    ...
                    <tbody>
                        @for ($i = 0; $i < count($minutesLeft); $i++) <tr
                            class="text-xs hover:bg-gray-100 odd:bg-white even:bg-gray-50">
                            <td class="px-4">{{ $minutesLeft[$i]['minute'] }}</td>
                            <td class="px-4">.{{ substr(explode('.', $minutesLeft[$i]['equiv'])[1], 0, 3) }}</td>
                            <td class="px-4">{{ $minutesRight[$i]['minute'] }}</td>
                            <td class="px-4">.{{ substr(explode('.', $minutesRight[$i]['equiv'])[1], 0, 3) }}</td>
                            </tr>
                            @endfor
                    </tbody>
                </table>
    ...
    <div class="w-[400px] flex flex-col gap-10">
        <form class="w-[400px] bg-white rounded-xl p-6 shadow" wire:submit.prevent="leavePay">
            ...
            <div>
                ...
                @error('leave_with_pay') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
            </div>
            <div class="mt-2">
                <label class="block text-sm text-gray-700">Leave without Pay <span class="font-bold text-sm">Day/Month
                        (1-30)</span></label>
                <input type="text" wire:model="leave_without_pay"
                    class="mt-1 block w-full h-10 border border-gray-200 bg-gray-50 rounded-md px-2 text-sm">
                @error('leave_without_pay') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
            </div>
            <button type="submit"
        ...
        <form class="w-[400px] bg-white rounded-xl p-6 shadow" wire:submit.prevent="hourDayBase">
            ...
            <div>
                ...
                <input type="text" wire:model="hour_day_base"
                    class="mt-1 block w-full h-10 border border-gray-200 bg-gray-50 rounded-md px-2 text-sm">
                @error('hour_day_base') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
            </div>
            <button type="submit"
                class="w-full bg-slate-700 text-white py-2 rounded-md hover:bg-slate-500 cursor-pointer mt-2 mb-2">
                Confirm
