    ...
    <div class="flex flex-col h-full w-1/2 bg-white p-6 rounded-xl shadow-sm">
        <form wire:submit.prevent="saveSignatory" class="w-full flex flex-col">
            ...
            <div class="mt-2">
                ...
                @error('approved_payment')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
                @enderror
            </div>

            <div class="mt-2">
                <label for="prepared_by2" class="block text-sm text-gray-700">Prepared by: (Payslip)</label>
                <select id="prepared_by2" wire:model="prepared_by2"
                    class="mt-1 block w-full h-10 border border-gray-200 bg-gray-50 rounded-md px-2">
                    <option value="" selected disabled>- - Select - -</option>
                    @foreach ($allSignatories as $signatory)
                        <option value="{{ $signatory->id }}">{{ $signatory->name }}</option>
        ...
        <div>
            ...
            <div class="w-full flex items-center">
                <p class="w-48">Prepared by: <i>(Payslip)</i></p>
                <div class="flex flex-col">
                    <p class="font-bold flex-1">{{ $assigned->prepared2->name ?? '-' }}</p>
                    <p class="text-xs text-gray-600 flex-1">{{ $assigned->prepared2->designation ?? '-' }}</p>
                </div>
            </div>
