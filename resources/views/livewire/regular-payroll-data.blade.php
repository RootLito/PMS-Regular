<div class="flex-1 flex-col relative gap-2">
    ...
    <div class="bg-white p-6 min-h-100 rounded-xl shadow-sm">
        ...
        <div id="payrollContent">
            ...
            <p class="text-xs font-bold uppercase mb-4">PERIOD COVERED: {{ $months[$month] ?? '' }} 1-30,
                {{ $year ?? '' }}</p>
            <table class="table-auto border-collapse w-full text-xs"
                ...
                <tbody>
                    @foreach ($employeesByOffice as $office => $employees)
                        ...
                        @foreach ($employees as $employee)
                            ...
                            <tr>
                                ...
                                @php
                                    $hasHDMF =
                                        (!empty($c->hdmf_ps) && $c->hdmf_ps != 0) ||
                                        (!empty($c->hdmf_mp2) && $c->hdmf_mp2 != 0);
                                @endphp
                                ...
                                <td class="border border-gray-300 px-2 py-1 text-right align-top">
                                    ...
                                    @else
                                        <div class="flex flex-col">
                                            <span>{{ !empty($c->hdmf_ps) && $c->hdmf_ps != 0 ? number_format($c->hdmf_ps, 2) : '' }}</span>
                                            <span>{{ !empty($c->hdmf_mp2) && $c->hdmf_mp2 != 0 ? number_format($c->hdmf_mp2, 2) : '' }}</span>
                                        </div>
                                    @endif
                                </td>
                                ...
                                <td class="border border-gray-300 px-2 py-1 text-right align-top">
                                    @php
                                        $total =
                                            collect($fieldsToCheck)->sum(fn($field) => $c->$field ?? 0) +
                                            ($c->tax ?? 0) +
                                            ($c->phic ?? 0) +
                                            ($c->gsis_ps ?? 0) +
                                            ($c->hdmf_ps ?? 0) +
                        ...
                        <tr>
                            ...
                            <td class="border-s border-gray-300 px-2 py-1 text-right">
                                {{ ($totalsByOffice[$office]['pera'] ?? 0) == 0 ? '-' : number_format($totalsByOffice[$office]['pera'], 2) }}
                            </td>
                        ...
                        <tr class="font-bold border-s border-e border-gray-300" style="background: #c6e1b4;">
                            ...
                            <td class=" px-2 py-1 text-right">
                                {{ ($totalsByOffice[$office]['tax'] ?? 0) == 0 ? '-' : number_format($totalsByOffice[$office]['tax'], 2) }}
                            </td>
                            <td class=" px-2 py-1 text-right">
                                {{ ($totalsByOffice[$office]['phic'] ?? 0) == 0 ? '-' : number_format($totalsByOffice[$office]['phic'], 2) }}
                            </td>
                            <td class=" px-2 py-1 text-right">
                            ...
                            <td class=" px-2 py-1 text-right">
                                @php
                                    $hdmf_total =
                                        ($totalsByOffice[$office]['hdmf_ps'] ?? 0) +
                            ...
                            <td class=" px-2 py-1 text-right">
                                {{ ($totalsByOffice[$office]['first'] ?? 0) == 0 ? '-' : number_format($totalsByOffice[$office]['first'], 2) }}
                            </td>
                            <td class=" px-2 py-1 text-right">
                                {{ ($totalsByOffice[$office]['second'] ?? 0) == 0 ? '-' : number_format($totalsByOffice[$office]['second'], 2) }}
                            </td>
                        </tr>
                    @endforeach
                    <tr class="">
                        ...
                        <td class="px-2 py-1 text-right">
                            {{ isset($overallTotal['grandTotalSalary']) && $overallTotal['grandTotalSalary'] != 0 ? number_format($overallTotal['grandTotalSalary'], 2) : '-' }}
                        </td>
                    ...
                    <tr class="">
                        ...
                        <td class="px-2 py-1 text-right">
                            {{ isset($overallTotal['otherTotal']) && $overallTotal['otherTotal'] != 0 ? number_format($overallTotal['otherTotal'], 2) : '-' }}
                        </td>
                        <td colspan="11" class="border-r border-gray-300 "></td>
                    </tr>
                    ...
                    <tr style="background-color: #f5b084; font-weight: bold;">
                        ...
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['grandTotal'] ?? 0) == 0 ? '-' : number_format($overallTotal['grandTotal'], 2) }}
                        </td>
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['grandTotal'] ?? 0) == 0 ? '-' : number_format($overallTotal['grandTotal'], 2) }}
                        </td>
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['tax'] ?? 0) == 0 ? '-' : number_format($overallTotal['tax'], 2) }}
                        </td>
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['phic'] ?? 0) == 0 ? '-' : number_format($overallTotal['phic'], 2) }}
                        </td>
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['gsis_ps'] ?? 0) == 0 ? '-' : number_format($overallTotal['gsis_ps'], 2) }}
                        </td>
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['ps_mp2'] ?? 0) == 0 ? '-' : number_format($overallTotal['ps_mp2'], 2) }}
                        </td>
                        <td></td>
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['totalOthers'] ?? 0) == 0 ? '-' : number_format($overallTotal['totalOthers'], 2) }}
                        </td>
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['totalDeduction'] ?? 0) == 0 ? '-' : number_format($overallTotal['totalDeduction'], 2) }}
                        </td>
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['netPay'] ?? 0) == 0 ? '-' : number_format($overallTotal['netPay'], 2) }}
                        </td>
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['firstHalf'] ?? 0) == 0 ? '-' : number_format($overallTotal['firstHalf'], 2) }}
                        </td>
                        <td class="px-2 py-1 text-right">
                            {{ ($overallTotal['secondHalf'] ?? 0) == 0 ? '-' : number_format($overallTotal['secondHalf'], 2) }}
                        </td>
                    </tr>
                    ...
                    <tr class="border-x border-b border-gray-300 ">
                        <td colspan="2" class="px-2 py-1 text-left">
                            {{ $this->assigned->prepared->designation ?? '' }}
                        </td>
                        <td colspan="3" class="px-2 py-1 text-left">
                            {{ $this->assigned->checked->designation ?? '' }}
                        </td>
                        <td colspan="3" class="px-2 py-1 text-left">
                            {{ $this->assigned->certified->designation ?? '' }}
                        </td>
                        <td colspan="3" class="px-2 py-1 text-left">
                            {{ $this->assigned->funds->designation ?? '' }}</td>
                        <td colspan="2" class="px-2 py-1 text-left">
                            {{ $this->assigned->approved->designation ?? '' }}
                        </td>
                    </tr>
                </tbody>
            </table>
